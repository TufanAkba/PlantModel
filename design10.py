...
# -*- coding: utf-8 -*-
"""
Created on Sun May  8 00:30:52 2022

@author: tufanakba
MG3 integration to basic brayton cycle

"""

# BUG : after W limit (diff<.001) set to .001 DESIGN goes infinite loop
...
if __name__ == "__main__":
    ...
    w_ini = mp_complex.get_val('DESIGN.inlet.Fl_O:stat:W')
    diff = 1
    ...
    while diff>0.001:
        ...
        p.set_val('receiver.Mass_Flow', mp_complex.get_val('DESIGN.inlet.Fl_O:stat:W', units='kg/s'), units='kg/s')
        p.set_val('receiver.T_fluid_in',mp_complex.get_val('DESIGN.rec.rec.T_fluid_in', units='K'), units='K')
        ...
        prob.set_val('DESIGN.rec.rec.s_RPC', p.get_val('receiver.s_RPC'))
        prob.set_val('DESIGN.rec.rec.s_INS', p.get_val('receiver.s_INS'))
        prob.set_val('DESIGN.rec.rec.L', p.get_val('receiver.L'))
        ...
        for pt in ['DESIGN']:
            viewer(prob, pt)
        w = mp_complex.get_val('DESIGN.inlet.Fl_O:stat:W')
    ...
    # print(f'Fn target:({fn})')
    # print(mp_complex.get_val('DESIGN.perf.Fn'))
