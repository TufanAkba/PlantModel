...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 10 21:24:36 2022
...
class Complex2(pyc.Cycle):
    ...
    def setup(self):
        ...
        if design:
            ...
            balance.add_balance('LPturb_PR', val=3., lower=2., upper=5., eq_units='W', rhs_name='pwr_target')
            self.connect('balance.LPturb_PR', 'LPturb.PR')
            self.connect('MG.P_el', 'balance.lhs:LPturb_PR')
            ...
            balance.add_balance('HPturb_PR', val=2.0, lower=1.25, upper=2.9, eq_units='hp', rhs_val=0.)
            self.connect('balance.HPturb_PR', 'HPturb.PR')
            self.connect('HPshaft.pwr_net', 'balance.lhs:HPturb_PR')
            ...
            balance.add_balance('HP_Nmech', lower=20000., upper=29000., units='rpm', eq_units='hp', rhs_val=0.)
            self.connect('balance.HP_Nmech', 'HP_Nmech')
            self.connect('HPshaft.pwr_net', 'balance.lhs:HP_Nmech')
            
            # balance.add_balance('Q_Solar', val=1000., upper=1100, lower=500, units='W',)
            # self.connect('balance.Q_solar', 'rec.Q_Solar_In')
            # self.connect('src_name', 'balance.lhs:Q_Solar')
            ...
            if self.options['setting_mode'] == 'nozzle':
                ...
                self.connect('balance.W', 'inlet.Fl_I:stat:W')
                self.connect('LPcomp.map.RlineMap', 'balance.lhs:W')
...
class MPComplex2(pyc.MPCycle):
    ...
    def setup(self):
        ...
        # Trials
        self.od_pts = ['Od170','Od170nozzle']
        self.od_pwrs = [170.,170.,180., 100.]
        self.Q_Solar = [970.,970.,1050., 750.]
        setting_mode = ['T4','nozzle','T4', 'nozzle']
        
        # self.od_pts = ['SelfSupplied','Motoring','Max_105', 'Min_75']
        # self.od_pwrs = [170.,170.,170., 170.]
        # self.Q_Solar = [900.,890.,960., 970.]
        # setting_mode = ['nozzle','nozzle','T4', 'T4']
        
        # self.od_pts = ['Min150','Min1502','Min1503','Min1504','Min1505']
        # self.od_pwrs = [-10.,-0.,-10.,-10.,-10.]
        # self.Q_Solar = [400.,400.,300.,200.,100.]
        # setting_mode = ['nozzle','nozzle','nozzle','nozzle','nozzle']
        
        # self.od_pts = ['Max200','Max190','Max180','Od170','Min160','Min150','Min140','Min130','Min120','Min110','Min100']
        # self.od_pwrs = [200.,190.,180.,170.,160.,150.,140.,130.,120.,110.,100]
        # self.Q_Solar = [1100.,1060.,1020.,980.,940.,890.,850.,800.,750.,710.,660.]
        # setting_mode = ['T4','T4','T4','nozzle','nozzle','nozzle','nozzle','nozzle','nozzle','nozzle','nozzle']
