...
class ReceiverSubProblem(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        if not opt:
            ...
            draw_contour(z_n[0,:], r_n[:,0], T-273, r_1+s_SiC, r_1+s_SiC+s_RPC, prob['receiver.Mass_Flow'], 15)
...
# for Mass_Flow in M:
    ...
    draw_contour(z_n[0,:], r_n[:,0], T-273.15, r_1+s_SiC, r_1+s_SiC+s_RPC, Mass_Flow, 10)
    
    Tf_out= float(p.get_val('receiver.T_fluid_out'))
    if Tf_out<1273:
